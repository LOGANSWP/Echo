...
## 照片派生文本的双语边界（4.0l / ADR-025，待实现）

OCR 保留识别原文及语言，画面描述标注机器生成、语言和模型/来源版本；二者不覆盖用户文本。机器描述输出仍遵循 zh-Hans/en-US 支持边界，创作按当前持久 UserPolicy.preferredLanguage 输出。用户切换语言不把旧语言缓存冒充当前输出；需要重新准备时按版本化任务处理，重试/重放计入获批预算。跨语言读取素材保留来源类型和原文，不在摄入 Pipeline 中调用展示层翻译。验收包含中英 OCR/机器描述/用户校正及语言切换后的真实无手写创作流程。

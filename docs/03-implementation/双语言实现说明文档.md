...
| 输出语言     | AI 响应正文必须匹配 UserPolicy.preferredLanguage；先校验 JSON envelope，再识别正文 | 有界协议校验 + NaturalLanguage + 简中脚本检查 | 每次模型调用最多 1 次语言重试，失败以类型化降级提示结束，不发布成功报告 |
...
flowchart TB
    ...
    subgraph Synthesize["🧠 合成路径 (语言受控输出)"]
        ...
        LLM["获批 bundled LLM（4.0k）"]
        RawResp["有界 JSON envelope<br/>结构与来源校验"]
        LangAlign["Language Aligner<br/>正文检测 + 每调用最多1次语言重试"]
        FinalResp["已校验生成结果<br/>或类型化降级提示"]
...
## 5. AI 动态内容的语言控制机制（ADR-023，2026-09-07 修订）

本节与 [ADR-023：离线生成的执行边界与可验证验收](../decisions/ADR-023-offline-generation-execution-contract.md) 一致，属于 `4.0k` 待实现和验证的合同，不表示当前生成式模型已获批或生产闭环已完成。

### 5.1 Prompt 语言指令注入

Response Synthesizer 在构造 Prompt 时，**显式注入语言控制指令**：

- 从持久 UserPolicy 读取 `preferredLanguage`（仅 `zh-Hans` 或 `en-US`）；系统语言映射只用于首次初始化，不在每次生成时覆盖用户已选语言。
...
- 将系统语言/结构指令与不可信来源内容明确分区，来源中的指令只作为数据，不获得系统指令权限。不得因来源出现“translate”等词就拒绝或改写来源原文；生成目的由可信模板定义。

### 5.2 Language Aligner 校验、重试与类型化结果

```mermaid
flowchart TD
    RawResp["有界模型调用返回 JSON"]
    Schema{"envelope 结构与上限校验通过?"}
    Provenance["按本次 allow-list 校验来源"]
    Detect["仅检测生成正文语言与简中脚本"]
    Match{"正文匹配 preferredLanguage?"}
    Pass["validated generation"]
    RetryAllowed{"本调用尚未语言重试且总预算允许?"}
    Retry["强化语言指令，重生成完整 envelope"]
    Fallback["language fallback / failure<br/>本地化提示，不发布成功报告"]
    Invalid["结构/超限失败 → L2<br/>不自动修复 JSON"]
    AuditLog["记录无原文语言结果与重试次数"]

    RawResp --> Schema
    Schema -->|Yes| Provenance
    Schema -->|No| Invalid
    Provenance --> Detect
    Detect --> Match
    Match -->|Yes| Pass
    Match -->|No| RetryAllowed
    RetryAllowed -->|Yes| Retry
    RetryAllowed -->|No| Fallback
    Retry --> RetryResp
    RetryResp["同输入 allow-list 与预算"] --> Schema
    Pass --> AuditLog
    Fallback --> AuditLog
    Invalid --> AuditLog
```

- **协议先行**：先限制原始字节数并解析/校验 envelope 的版本、结构、段落、字符与引用数，再校验该调用的实际输入 allow-list；未知版本、畸形或超限输出直接 L2，不展示未校验内容。合法结构中未知/未提交 ID 不成为锚点，按 US-SYN-002 AC-3 标记 `noSource/partialNoSource`，不因此一律拒绝整篇。
- **正文检测**：使用 iOS 18 NaturalLanguage 识别生成标题/正文的主体语言；JSON keys、UUID 与协议引用标签不作为正文语言样本。专名及明确标注的原文引用由冻结验收集标注与人工复核评判，不得用模型自我标注或宽泛正则删除正文绕过运行时检测。不确定或没有可识别自然语言不能直接算匹配成功；`zh-Hans` 另需简中脚本检查，不能把所有中文检测结果等同简体。主体语言匹配不等于每个词都已翻译或正文可读，简繁转换信号不授权改写正文/来源。
- **重试上限**：每个有界模型调用（叶子或 reduce）最多 **1 次语言重试**，即最多两次推理；重试重新生成完整 envelope，并重新验证相同实际输入 allow-list、schema 与 token 预算，不能只替换正文并保留旧锚点。schema 失败不增加 JSON 修复轮次，通用 L1 退避也不能扩大语言重试次数。
- **总预算**：单次逻辑报告执行尝试的模型调用数和时间有总上限；叶子、reduce、语言重试与恢复重放均计入。新一次用户明确 Continue/Restart 或合法系统资源延后的预算边界显式记录，内部循环不得重置预算。
- **类型化结果**：区分 `validated generation`、`language fallback`、`failure/cancelled/resource-deferred`。语言重试耗尽时以 L2 暴露可手动重试的真实失败，并展示跟随 `preferredLanguage` 的固定本地化降级提示；提示不含 LLM 成分，不写成功报告、`completed` 或 generated 审计。
- **无数据与无输出分离**：`noData` 仅表示当前没有可用输入。有真实输入却得到空 `paragraphs`/空正文是生成失败；合法空 `sourceMemoryIDs` 是 `noSource`，不能借此记为 `noData`。
- **审计全覆盖**：成功、重试成功与降级都记录无原文的语言结果和 `languageRetryCount`（每调用 0 或 1）；中间调用成功不能作为最终报告完成证据。

### 5.3 混合语言上下文处理

当检索结果包含多种语言时，Synthesizer 的处理策略：

- **不翻译上下文**：将当前授权的有界多语言摘录置于不可信来源分区，依赖 LLM 的跨语言理解能力；不修改持久来源原文，截断在 coverage 中明确记录。
...
- 每层只接收实际输入的来源链：叶子调用使用该批 opaque MemoryID，reduce 的 allow-list 只来自实际消费的已校验子结果。v1 reduce 仅消费 `cited` 子段落；中间 `noSource/partialNoSource` 被省略并计入 coverage，不能通过复制其他来源 ID 洗成有据文本。最终层的无来源状态仍按产品合同诚实展示。
- tokenizer 预算覆盖完整渲染后的系统指令、chat template、schema、来源、术语与重试指令；每次满足 `inputTokens + reservedOutputTokens <= approvedContextTokens`。字节和字符上限继续保留，不能替代 token、输出、总调用或驻留内存限制。
- 同 taskId 的跨进程 Continue 先重新授权，按模型/tokenizer/chat-template、算法/限制、目标语言和输入摘要重建计划；保留持久 checkpoint，在内存重放所需前缀，不把游标当作已存在的模型输出，也不把重放报作新增进度。身份变化提示 Restart；工件不合法仍走 L3，权限拒绝仍走 denial。
- 不在恢复描述中保存来源原文、生成草稿、KV cache 或授权快照；每次调用与最终 publication 前重验当前授权及来源版本。coverage 统计实际消费的唯一来源和实际层数，不把预先选择的输入或计划层数当成生成完成。
...
`4.0k` 的现行生成/语言执行合同见 [ADR-023](../decisions/ADR-023-offline-generation-execution-contract.md)（2026-09-07）；下表保留早期跨语言设计索引，不构成具体 LLM 工件批准。
...
## 照片派生文本的双语边界（4.0l / ADR-025）

2026-09-09 人类要求功能闭环优先：不要求一个视觉模型直接承担所有最终语言能力后才能开始接线。可在研究中验证 en-US 机器观察作为带类型/版本的中间素材，经既有本地生成器形成 preferredLanguage 的最终输出；必须计入实际调用预算并保留中间语言，不能改语言标签冒充中文成功。用户可见的生成描述/创作仍遵循当前语言校验，真实链路未接通则报告缺口。该组合研究不等于新生产工件批准，质量失败另记待办。

OCR 保留识别原文及语言，画面描述标注机器生成、语言和模型/来源版本；二者不覆盖用户文本。机器描述输出仍遵循 zh-Hans/en-US 支持边界，创作按当前持久 UserPolicy.preferredLanguage 输出。用户切换语言不把旧语言缓存冒充当前输出；需要重新准备时仅在下一次用户访问详情/所选照片创作时按版本化任务处理，重试/重放计入获批预算。跨语言读取素材保留来源类型和原文，不在摄入 Pipeline 中调用展示层翻译。验收包含中英 OCR/机器描述/用户校正及语言切换后的真实无手写创作流程。

**按需触发（2026-09-09）**：仅用户打开该照片详情或选择该照片进行 AI 创作时，才按需准备画面描述/OCR 创作素材；导入、同步、前后台及自动报告扫描不发起全相册理解。有效结果复用，失败仅显式重试。 导入阶段既有检索 OCR 与向量化保持原路径；本条约束视觉描述及创作素材准备，不把语言切换当成全相册重新推理入口。

...
3. **`4.0i` 引用与分享**：生成模型只接收 opaque MemoryID，并输出版本化、受字节/段落/字符/引用数上限约束的 JSON envelope。parser 只接受当前策略过滤后实际提交给模型的输入 allow-list；未知版本、超限或结构损坏整体 L2 fail-closed，禁止位置轮询。每段用 `[SourceAnchor]` 与 `cited/noSource/partialNoSource` 表达 provenance；allow-list 命中不等于事实已验证。锚点导航，以及复制/Markdown/PDF/plain-text 分享准备，均重新校验当前 UserPolicy 与来源可解析性；撤权阻止派生正文交接。所有格式保留可理解引用，多页 PDF 不截断。只有系统控制器实际呈现回调后才写 `.creationSharePresented(sharePresented=true)`；payload 就绪不是呈现证据，审计写入失败不得把已发生的呈现改写为 false。完整边界见 ADR-020。

...
审计日志写入独立 SQLite 表 `AuditLog`，由 `PrivacyActor` 负责。字段遵循规格书附录 15，保留最近 30 天数据，超期自动清理（由后台任务每日执行）。`4.0i` 通过幂等 migration 为 `AuditLog`/`AuditLogEntry` 增加 `templateType`、`sourceMemoryCount`、`citationCount`、`noSourceCount`、`exportFormat`、`sharePresented` 与 `periodType` 类型化字段；`exportFormat` 只允许 `plainText|markdown|pdf`，`periodType` 只允许 `month|year`。禁止继续把创作/分享 metadata 编码进 `sourceLanguage`、`contentHash` 或自由文本。

...
4. **`4.0j` 叙事报告调度**：composition-owned scheduler 使用分别持久化且新安装默认开启的月/年开关，并为两类周期分别保存 `eligibleFrom`。新安装在同意已持久化且首条可用 canonical memory 落库时同时设置两类基线，升级安装在本版本首次成功迁移时设置；关闭后重开只重设对应类型基线，不影响另一类仍开启的调度，避免无限历史补算。月报/年报对应刚结束的完整公历周期；首次物化冻结日历、时区与 UTC 半开区间。每次启动、前台、获准 `BGProcessingTask` 或用户明确动作最多 compare-and-set claim 一个最早周期，相同结束边界先月后年。`NarrativeReportActor` 通过 `DatabaseManager` 管理 `NarrativeReportSchedule`、`NarrativeReportPeriod`、版本化有界 `NarrativeReport` 与 `NarrativeReportSource`；成功 publication 在单一无挂起点事务中提交报告、来源关系、周期完成和准备好的结构化审计记录。任务经 `TaskQueueActor` 串行，`ProgressActor` 仅保存有界恢复描述；L2 仅用户重试，系统 expiration/资源不足释放 claim 后等待下一合法机会。无数据写 `noData`；用户删除报告或来源删除将派生报告删除并标记 v1 不可重建的 `invalidated`。详见 ADR-021。

`4.2`、`4.5` 等后续任务只消费上述接口并验证，不得在测试中新增生产语义。编辑与任务拆分由 ADR-017 管辖；来源解析、PhotoKit 外部结果门禁和删除恢复由 ADR-019 管辖；引用协议、授权后交接与分享审计由 ADR-020 管辖；叙事报告周期、持久化、后台机会、L2 与派生数据删除由 ADR-021 管辖。

...
### 2.6 Focus 生产动作拆分（Phase 4 `4.0e/4.0h/4.0i/4.0j/4.0k`）
...
5. **`4.0k` 端侧生成运行时与报告生成闭环**：按 ADR-009 完成获批不可变 bundled LLM 工件、许可证/SBOM/校验和登记、生产 `LLMProvider` 与 `LanguageAligner` 装配，并真实消费 `sourceBatches` 完成确定性有界分批/分层聚合；只有 no-fixture、真实工件端到端证据才可宣称 US-SYN-004 自动生成完成。详见 ADR-022。

...
### 2.6 Focus 生产动作拆分（Phase 4 `4.0e/4.0h/4.0i/4.0j/4.0k/4.0l`）
...
2. **`4.0h` 来源解析与删除**：统一 `FocusSourceResolver` 返回分面的 Sendable 结果，分别表达内容可用性、展示方式与原始删除能力。PhotoKit photo/video 只有在当前 UserPolicy 允许、`.readWrite` 为 authorized/limited、资产在当前范围可获取且 `PHAsset.canPerform(.delete)` 时才可请求系统删除；内容未下载与能否删除是两个独立维度。Share Extension 文本展示 canonical 内容，音频按 US-ING-003 只展示转写，二者均不声称来源 App 身份或删除能力。`MemoryDeletionJournal` 在既有 D-005 `phase` 外增加 `intentKind` 与 `sourceDeletionState`；`photoLibraryAndEcho` 只有在外部结果持久化为 `confirmedDeleted` 后才能从 `.planned` 推进本地清理。恢复时 full authorized 的确认缺失与 limited/撤权下不可见严格区分，后者保持数据并进入 L2。详见 ADR-019。
3. **`4.0i` 引用与分享**：生成模型只接收 opaque 来源身份（领域 MemoryID 或请求内无损别名），并输出版本化、受字节/段落/字符/引用数上限约束的 JSON envelope。parser 只接受当前策略过滤后实际提交给模型的输入 allow-list；未知版本、超限或结构损坏整体 L2 fail-closed，禁止位置轮询。每段用 `[SourceAnchor]` 与 `cited/noSource/partialNoSource` 表达 provenance；allow-list 命中不等于事实已验证。锚点导航，以及复制/Markdown/PDF/plain-text 分享准备，均重新校验当前 UserPolicy 与来源可解析性；撤权阻止派生正文交接。按 ADR-024，复制及所有导出默认只含标题和正文，不附加内部来源标识；引用与无来源状态保留在 App 内，多页 PDF 不截断。只有系统控制器实际呈现回调后才写 `.creationSharePresented(sharePresented=true)`；payload 就绪不是呈现证据，审计写入失败不得把已发生的呈现改写为 false。完整边界见 ADR-020。
4. **`4.0j` 叙事报告调度与存储基础**：composition-owned scheduler 使用分别持久化且新安装默认开启的月/年开关，并为两类周期分别保存 `eligibleFrom`。新安装在同意已持久化且首条可用 canonical memory 落库时同时设置两类基线，升级安装在本版本首次成功迁移时设置；关闭后重开只重设对应类型基线，不影响另一类仍开启的调度，避免无限历史补算。月报/年报对应刚结束的完整公历周期；首次物化冻结日历、时区与 UTC 半开区间。每次启动、前台、获准 `BGProcessingTask` 或用户明确动作最多 compare-and-set claim 一个最早周期，相同结束边界先月后年。`NarrativeReportActor` 通过 `DatabaseManager` 管理 `NarrativeReportSchedule`、`NarrativeReportPeriod`、版本化有界 `NarrativeReport` 与 `NarrativeReportSource`；成功 publication 在单一无挂起点事务中提交报告、来源关系、周期完成和准备好的结构化审计记录。任务经 `TaskQueueActor` 串行，`ProgressActor` 仅保存有界恢复描述；L2 仅用户重试，系统 expiration/资源不足释放 claim 后等待下一合法机会。无数据写 `noData`；用户删除报告或来源删除将派生报告删除并标记 v1 不可重建的 `invalidated`。生产生成器缺失时在周期物化/claim 前返回 `generationUnavailable`，不得创建 L2。详见 ADR-021/022。
5. **`4.0k` 端侧生成运行时与报告生成闭环**：按 ADR-009 完成获批不可变 bundled LLM 工件、许可证/SBOM/校验和登记、生产 `LLMProvider` 与 `LanguageAligner` 装配，并真实消费 `sourceBatches` 完成确定性有界分批/分层聚合；该任务证明文本输入的真实工件端到端生成；完整照片输入还依赖 4.0l，不能仅凭文本或手写描述样本宣称全部照片报告/创作完成。详见 ADR-022。

**4.0k 执行合同（ADR-023，2026-09-07）**：先形成模型/运行时候选与可审阅预算，再由人类按确切 hash/许可/设备 scope 审批，之后生产装配；ModelManifestActor 只登记身份。运行时 Actor 隔离每请求 KV cache，Pipeline 保持无可变会话状态；确定性指计划/请求，不要求跨硬件逐 token 一致。完整 prompt 按获批 tokenizer 计数，输入加预留输出不超过 context，并约束输出、层级、fan-in、总调用/时间与驻留内存。每层 allow-list 只来自该层实际消费的叶子来源；reduce 仅消费 cited 子段落，省略/截断写真实 coverage；派生关系覆盖实际贡献输入而非只取最终锚点。先解析 envelope，再对正文执行每调用最多一次 LanguageAligner 重试，降级提示不 publication。Continue 保留 checkpoint 并重建丢失的内存聚合前缀；输入/模型/算法身份变化要求 Restart，最终再次核验当前授权/内容版本。具体错误矩阵、质量样本与实机证据见 ADR-023 §4~6。

**中文诗传输优化（2026-09-08 / ADR-023）**：来源身份在领域中仍为 canonical MemoryID；模型传输可使用显式 `requestAliasV1`，由不可变 `GenerationReferenceMap` 为当前真实输入建立一一对应的短别名。运行时只按模型明确返回的别名查表还原，不改变正文、不默认绑定、不扩大 allow-list。未知别名或超限输出按 L2 拒绝；还原后继续领域解析、语言检查及真实来源复验。示范 ID 不属于映射，别名不持久化，重试保留同一编码，prompt/编码变更进入恢复身份。该优化首先用于中文诗；报告与其他请求保留 UUID 传输。

`4.2`、`4.5` 等后续任务只消费上述接口并验证，不得在测试中新增生产语义。编辑与任务拆分由 ADR-017 管辖；来源解析、PhotoKit 外部结果门禁和删除恢复由 ADR-019 管辖；引用协议、授权后交接与分享审计由 ADR-020 管辖；叙事报告周期、持久化、后台机会、L2 与派生数据删除由 ADR-021 管辖；生成运行时门禁与 4.0j/4.0k 证据拆分由 ADR-022 管辖。

------

### 2.7 照片按需理解与创作素材（4.0l）

按 [ADR-025](../decisions/ADR-025-photo-understanding-creation.md)，图片摄入仅建立检索索引；用户打开所选照片详情或将其用于 AI 创作时，才通过现有 TaskQueue/Progress 进行有界本地理解。导入、同步及前台扫描不自动批量准备，自动报告只消费已就绪素材。OCR 和机器画面描述属于两种独立派生内容，由 DatabaseManager 管辖的 Actor 关联原 MemoryID 持久化版本、状态和语言，不覆盖 canonicalText 或用户编辑。创作/报告消费带类型和版本的素材；用户描述/校正优先，没有手写内容时自动使用机器描述，标题/标签不屏蔽画面描述。

每次读取/推理/发布重验授权与来源版本。图片变化、排除/确认删除及撤权贯穿派生数据、表示/缓存、恢复描述和报告 D-005 关系；UI 仅投影状态并显示机器来源标记，不持有第二份领域真相。模型选择和预算需单独审批，既有 SigLIP2 检索向量和文本 LLM 不构成图像描述能力。当前代码没有该闭环；具体 schema/迁移及生产接线属于 4.0l。
...
审计日志写入独立 SQLite 表 `AuditLog`，由 `PrivacyActor` 负责。字段遵循规格书附录 15，保留最近 30 天数据，超期自动清理（由后台任务每日执行）。`4.0i` 通过幂等 migration 为 `AuditLog`/`AuditLogEntry` 增加 `templateType`、`sourceMemoryCount`、`citationCount`、`noSourceCount`、`exportFormat`、`sharePresented`、`periodType` 与 hash-only `shareHandoffIdDigest` 类型化字段；`exportFormat` 只允许 `plainText|markdown|pdf`，`periodType` 只允许 `month|year`。`.creationSharePresented` 以 `shareHandoffIdDigest` partial unique index 和精确查询保证无窗口幂等；无效周期在持久化与重试入队前拒绝。禁止继续把创作/分享 metadata 编码进 `sourceLanguage`、`contentHash` 或自由文本。
...
手动创作由应用级 CreationLibraryActor 经 TaskQueueActor 调度，DatabaseManager 事务保存任务、成功正文和全部贡献来源关系；UI 仅投影状态。页面消失不拥有取消权，显式取消才协作终止。重启中断仅手动重新生成。D-005 与当前授权重验贯穿读写导出。

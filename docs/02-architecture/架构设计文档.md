...
### 2.7 照片自动理解与创作素材（4.0l，待实现）

按 [ADR-025](../decisions/ADR-025-photo-understanding-creation.md)，图片摄入后通过现有 TaskQueue/Progress 进行有界本地理解；图片本身先完成导入。OCR 和机器画面描述属于两种独立派生内容，由 DatabaseManager 管辖的 Actor 关联原 MemoryID 持久化版本、状态和语言，不覆盖 canonicalText 或用户编辑。创作/报告消费带类型和版本的素材；用户描述/校正优先，没有手写内容时自动使用机器描述，标题/标签不屏蔽画面描述。

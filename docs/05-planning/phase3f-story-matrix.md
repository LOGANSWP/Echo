...
> **4.0k specification review (2026-09-07, ADR-023):** SYN-001/002 are explicit generation prerequisites alongside SYN-003/004. The baseline labels below remain historical; production model, structured language alignment, per-layer provenance, bounded replay and actual device evidence are still owned by 4.0k and are not claimed complete by this documentation change.
...
| ING-004 | Impossible/spec-invalid | 3F.0, 3F.3, 3F.3a, 3F.5, 4.2, 4.0l | Some ingest AC code is tested with StubEmbedder, but required direct text/vision-space alignment conflicts with the mandated E5/CLIP split. 3F.3a owns SigLIP2 Core ML conversion and real vision inference. ADR-025 adds unimplemented photo understanding/derived text and no-manual-description creation in 4.0l; historical baseline does not cover these new ACs. |
...
| SYN-001 | Stub | 3F.9, 3F.10, 4.0k, 4.2 | Baseline: fixture language transition. 3F.10 supplies the persisted shared language choice; 4.0k must prove real model prompt/body-language alignment, one retry per call, non-publishing fallback and frozen quality metrics under ADR-023. |
| SYN-002 | Partial | 3F.9, 4.0i, 4.0k, 4.2 | 3F.9 renders anchors but assigns them by paragraph position. ADR-020 requires 4.0i to parse a versioned bounded JSON envelope, accept only model-emitted opaque source IDs from the exact currently authorized submitted-input allow-list, express multiple anchors plus cited/noSource/partialNoSource, and route anchors through current-policy revalidation. Provenance membership is not presented as automatic fact verification.  ADR-023 assigns real per-layer generation and evidence to 4.0k. |
| SYN-003 | Partial | 3F.0, 3F.9, 4.0i, 4.0k, 4.2, 4.0l | Private Notes creation/deep-link was replaced by user-mediated system share/export. 4.0i owns current-policy revalidation for copy/export/share, title/body-only plain text/Markdown/multipage PDF under ADR-024, with internal provenance retained, actual share-controller presentation evidence, and dedicated typed creation/share audit fields.  ADR-023 assigns real per-layer generation and evidence to 4.0k. ADR-025 adds unimplemented photo understanding/derived text and no-manual-description creation in 4.0l; historical baseline does not cover these new ACs. |
| SYN-004 | Partial | 3F.0, 3F.9, 4.0i, 4.0j, 4.0k, 4.2, 4.0l | 4.0i owns provenance-honest citations and authorized share handoff. Per ADR-021/022, 4.0j owns persisted monthly/yearly switches and independent eligibleFrom baselines, completed periods/frozen bounds, one-period CAS, report/source persistence, atomic publication, noData/manual-L2/BG/D-005, plus generator-unavailable fail-closed-before-claim. 4.0k owns the approved bundled LLM, production provider/Language Aligner, real bounded batched/layered generation and no-fixture E2E. Re-enabling one type cannot change the other baseline; Stub reports are not production generation evidence. ADR-025 adds unimplemented photo understanding/derived text and no-manual-description creation in 4.0l; historical baseline does not cover these new ACs. |
...
| AWK-005 | Partial | 3F.7, 3F.8, 4.0d, 4.0h, 4.0i, 4.2 | 4.0d owns real card media/music/feelings and card→typed-Focus routing. ADR-019 assigns typed Focus source resolution and PhotoKit deletion capability to 4.0h; Share audio is transcript-only because the original is not persisted. 4.0i owns current-policy-revalidated source-anchor navigation without claiming semantic fact proof, and 4.2 verifies the joined loop. |
...
| PRV-004 | Partial | 3F.1, 3F.4, 3F.7, 4.0h, 4.2, 4.5, 4.9 | Remove-from-Echo is connected. ADR-019 limits source deletion to visible PhotoKit photo/video that pass `canPerform(.delete)` and requires a persisted external-result gate before D-005; limited-hidden and revoked sources cannot be inferred deleted. Share Extension sources cannot be deleted by Echo. |
...
| PRV-007 | Impossible/spec-invalid | 3F.0, 3F.4, 4.0h, 4.2, 4.5, 4.9 | The immutable 2026-08-03 baseline reflects the former impossible five-second wall-clock promise. ADR-019 now replaces it with verifiable PhotoKit delivery/full-scope reconciliation and recoverable D-005 cleanup; current delivery is Partial. Share Extension originals remain unobservable and limited-hidden is not deletion evidence. |

...
- **SYN-004 — Phase 3F audit result (superseded by ADR-017 and refined by ADR-021):** The original exact-schedule/People/Notes formulation was not implementable. Phase 4 task `4.0j` owns persisted default-on schedule switches and independent per-period-type eligibleFrom baselines, completed-calendar periods with frozen boundaries, one-period CAS claiming, bounded report/source persistence, atomic completion/audit publication, manual-only L2 retry, opportunistic BGProcessingTask handling, and D-005 invalidation. Re-enabling one period type never changes the other type's baseline. It omits unavailable partitions and never claims exact background execution.

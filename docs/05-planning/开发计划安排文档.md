...
| 4.0j | 月度与年度叙事报告持久调度闭环 | 完整周期/启用基线、CAS claim、报告/来源持久化、TaskQueue/Progress、原子审计与 D-005 | 4.0i |
...
`4.0j` 按 ADR-021 交付生产闭环：月/年调度分别使用持久开关且新安装默认开启，并分别持久化 `monthlyEligibleFrom` / `yearlyEligibleFrom`；新安装在同意持久化并有首条可用 canonical memory 时、升级安装在本版本首次成功迁移时分别设置基线，关闭后重开只更新对应类型的新基线，不得截断另一类型，以阻止无限补算。只为刚结束的完整公历周期建立 `periodType + periodKey`，首次物化冻结日历、时区与 UTC 半开边界。启动、前台、获准 `BGProcessingTask` 或用户动作每次最多 CAS claim 一个最早 `eligible` 周期，相同结束边界先月后年。新增 `NarrativeReportSchedule`、`NarrativeReportPeriod`、版本化有界 `NarrativeReport`、`NarrativeReportSource` 与 Actor/repository；任务经 `TaskQueueActor` 串行，`ProgressActor` 只存有界游标和版本。成功 publication 在 DatabaseManager 无挂起点事务中原子写报告、来源关系、完成状态与 `.narrativeReportGenerated`；无数据为 `noData`，L2 为只可手动重试的 `retryRequired`，系统 expiration/低电量/thermal 只释放 claim 延后。聚合受版本化 limits 约束，不可用人物/HealthKit 分区省略并标注；用户删除报告、来源删除与全量撤权纳入 D-005，`invalidated` 在 v1 不可重建。测试文件为 `EchoTests/Phase4/4.0j_NarrativeReportSchedulerTests.swift`。

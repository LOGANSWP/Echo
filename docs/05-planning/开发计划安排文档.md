...
**生效日期**：2026-06-15（最后更新：2026-09-05）
...
- `3F.finalize` 最初解锁 14 个发布资格任务（4.1..4.14）；2026-08-31 人类批准全 App 平衡画布后新增任务族 `4.0`/`4.0a`/`4.0b`/`4.0c`；后续规格审查新增 `4.0d`~`4.0g`。2026-09-03 ADR-017 将过大的 `4.0e` 收敛为编辑/冲突，并新增来源删除 `4.0h`、引用/分享 `4.0i` 与叙事调度 `4.0j`；2026-09-07 PR #78 预审依 ADR-022 再新增真实离线生成运行时 `4.0k`，2026-09-08 ADR-025 再新增照片自动理解与创作输入 `4.0l`，Phase 4 共 27 个任务。消费最终 UI 的下游任务按实际范围依赖对应生产边界，测试任务不得补写功能
...
> Phase 4 以发布资格为主，不重复开发 Phase 3F 已完成的基本业务闭环。新增产品范围包括平衡画布 `4.0`~`4.0c`，以及规格审查确认尚未完成的生产边界 `4.0d`~`4.0l`；其余任务负责验证、加固和发布。只有 `4.0`~`4.0l` 与 `4.1`~`4.9` 全部完成后才能生成唯一 RC。`task-status.json` 是任务状态、依赖与证据字段的唯一权威。
...
| 4.0i | 可验证引用、稳定导航与系统分享审计闭环 | 有界 JSON、多引用 provenance、当前授权交接、真实呈现与 typed audit | 4.0h |
| 4.0j | 月度与年度叙事报告持久调度与存储基础 | 完整周期/启用基线、CAS claim、报告/来源持久化、TaskQueue/Progress、原子审计、D-005 与 generator-unavailable fail-closed | 4.0i |
| 4.0k | 端侧生成式模型运行时与叙事报告生成闭环 | 获批 bundled LLM、生产 LLMProvider/Language Aligner、真实有界分批/分层聚合与 no-fixture E2E | 4.0j |
| 4.0l | 照片按需理解与无需手写描述的创作闭环 | 本地画面描述/OCR 正文、独立派生存储、新旧照片处理、手动创作/报告输入与 D-005 | 4.0k |
| 4.1 | Golden Dataset 与反馈排序质量验收 | 800+ 跨语言、200+ 反馈、100+ 术语用例及 Recall@10 报告 | 3F.11 |
| 4.2 | 全 Pipeline 生产端到端集成测试 | 扩展 no-fixture 正/负向、迁移与重启恢复矩阵，并覆盖跨 Surface 旅程 | 4.0a~4.0l |
| 4.3 | 性能基准与资源门禁验证 | P95、内存、CPU、energy、thermal 及 Discovery→Focus launch/hitch 原始证据 | 4.0a, 4.0b, 4.0k, 4.0l |
| 4.4 | 低电量、过热与模型加载恢复验证 | 降级、L1~L4、取消/恢复与 Task surface 运行时状态 | 4.0c, 4.0g, 4.0k, 4.0l |
| 4.5 | 数据删除与 ExcludedAssets 边界验收 | D-005 原子清除、PhotoKit saga、级联删除与恢复边界 | 4.0h, 4.0l |
| 4.6 | 覆盖率、静态分析与隐私门禁复核 | 含全部页面族及生产闭环新代码的全局覆盖率 >=95%、PrivacyCheckpoint 100%、静态禁令 | 4.0a~4.0l |
| 4.7 | 双语、本地化与无障碍验收 | 全 App 双语 100%、AX、Dynamic Type、对比度、family 回退与等价动作 | 4.0a~4.0l |
| 4.8 | 完成项目与发布文档更新 | 规格/架构/数据流/全 App UI 行为/发布文档与 deferred 证据互链 | 4.0a~4.0l |
| 4.9 | 发布合规、归档与签名验证 | 受控 signed archive/export、entitlements、SBOM、零外传 | 3F.11, 4.0f, 4.0k, 4.0l |
| 4.10 | 生成并批准 Release Candidate | 唯一 RC 身份与 Release Manager 批准；DEF-001/002 必须先处置 | 4.0~4.0l, 4.1~4.9 |
...
| 4.14 | Phase 4 集成测试：质量保障与最终发布验证 | Phase 1~4 累计测试、最终 RC 与人类发布批准 | 4.0~4.0l, 4.1~4.13 |
...
`4.0h` 交付 Focus 类型化来源能力和 US-PRV-004/007 PhotoKit 删除边界。resolver 分开表达内容可用性、展示方式和删除能力；Share Extension text 展示 canonical，audio 只展示转写，不猜测 host App。只有 UserPolicy 允许、authorized/limited-visible、资产可获取且 `PHAsset.canPerform(.delete)` 的 photo/video 显示“同时删除原始文件”。同一 `MemoryDeletionJournal` 正交保存外部 `sourceDeletionState` 与本地 D-005 `phase`；请求前为 prepared，只有 confirmedDeleted 解锁清理。full 缺失可补偿确认，limited-hidden/撤权为 indeterminate 并保留 Echo 数据。审计字段结构化，ExcludedAssets 提示只有实际展示后才能记录 `userNotified=true`。测试文件为 `EchoTests/Phase4/4.0h_FocusSourceLifecycleTests.swift`，完整边界见 ADR-019。

##### 4.0i 可验证引用、稳定导航与系统分享审计闭环

`4.0i` 按 ADR-020 交付版本化、受字节/段落/字符/引用数上限约束的 JSON 生成协议，模型只接收 opaque MemoryID；parser 只接受当前策略过滤后实际提交模型的精确 allow-list，未知版本、畸形或超限整体 L2 fail-closed，禁止 round-robin。领域/UI 使用 `[SourceAnchor]` 与 `cited/noSource/partialNoSource`，且不得把 provenance 命中声称为事实验证。锚点导航，以及复制/plain text/Markdown/PDF/share 准备，均在当前 UserPolicy + PrivacyCheckpoint 下复验；撤权阻止派生正文交接，允许但离线的来源在 App 内保留诚实标记；按 ADR-024，所有外部格式默认只含标题和正文，不附加来源 ID，PDF 多页不截断。`.creativeGeneration` 与 `.creationSharePresented` 分离；只有系统控制器真实呈现回调写 `sharePresented=true`，呈现后的审计失败不得改写事实。每次 handoff 以 hash-only `shareHandoffIdDigest`、唯一索引和精确查询幂等；无效 periodType 不得进入持久化重试。审计使用专用 typed columns，不记录 activityType、目标 App、用户完成状态、原文或 locator。测试文件为 `EchoTests/Phase4/4.0i_GroundedCitationShareTests.swift`。

##### 4.0j 月度与年度叙事报告持久调度与存储基础

`4.0j` 按 ADR-021/022 交付可独立验证的调度与持久化基础：月/年调度分别使用持久开关且新安装默认开启，并分别持久化 `monthlyEligibleFrom` / `yearlyEligibleFrom`；新安装在同意持久化并有首条可用 canonical memory 时、升级安装在本版本首次成功迁移时分别设置基线，关闭后重开只更新对应类型的新基线，不得截断另一类型，以阻止无限补算。只为刚结束的完整公历周期建立 `periodType + periodKey`，首次物化冻结日历、时区与 UTC 半开边界。启动、前台、获准 `BGProcessingTask` 或用户动作每次最多 CAS claim 一个最早 `eligible` 周期，相同结束边界先月后年。新增 `NarrativeReportSchedule`、`NarrativeReportPeriod`、版本化有界 `NarrativeReport`、`NarrativeReportSource` 与 Actor/repository；任务经 `TaskQueueActor` 串行，`ProgressActor` 只存有界游标和版本。注入 generator 后的成功 publication 在 DatabaseManager 无挂起点事务中原子写报告、来源关系、完成状态与 `.narrativeReportGenerated`；无数据为 `noData`，L2 为只可手动重试的 `retryRequired`，system expiration/资源不足只释放 claim 延后。生产 generator 未装配时必须在物化/claim 前返回 `generationUnavailable`，不写 L2、报告或审计。用户删除报告、来源删除与全量撤权纳入 D-005，`invalidated` 在 v1 不可重建。Stub 仅证明 seam，不证明生产生成。测试文件为 `EchoTests/Phase4/4.0j_NarrativeReportSchedulerTests.swift`。

##### 4.0k 端侧生成式模型运行时与叙事报告生成闭环

`4.0k` 按 ADR-009/022 选择并审批可商业分发的端侧生成式模型与运行时，固定不可变 revision、SHA-256、许可证、NOTICE、SBOM 与转换 lineage，随 App 打包且运行时零网络。任务实现并装配生产 `LLMProvider` 与 `LanguageAligner`，真实消费 `sourceBatches` 完成确定性有界分批/分层聚合，保持每段 `sourceMemoryIDs[]`、current-policy allow-list、`cited/noSource/partialNoSource` 与 R-004。验收必须使用真实 bundled artifact 的 no-fixture 端到端生成、重启、取消、资源与语言测试；在该任务完成前 US-SYN-004 保持 Partial，任何 RC/全 Pipeline 测试不得把 Stub 当生产证据。测试文件为 `EchoTests/Phase4/4.0k_OfflineGenerationRuntimeTests.swift`。

**2026-09-07 合理性复审（ADR-023）**：本任务先交付可审阅候选与质量/资源/设备预算材料，再取得具体模型、运行时、tokenizer/template 和完整文件摘要的人类审批，最后实现生产接线；ready 仅表示账本依赖就绪。直接验收包含 US-SYN-001/002/003/004，继承 US-RES-004 模型失败行为。不预先指定未经批准的模型，不把 ModelManifestActor 或旧 verify-only 结果当成审批。具体 token/context、输出、层数、调用、时间、KV/峰值内存和包体预算须在候选审批材料冻结；未取得时不得宣布生产可交付。

实现必须按 ADR-023 逐层保留真实来源身份和派生删除依赖，先解析 JSON 再校验正文语言；每调用最多一次语言重试，总预算包含重试/恢复重放，降级提示或空输出不写成功报告。Continue 保留原 checkpoint 并重建必要内存前缀，模型/输入/算法身份变化要求合法 Restart。验收分合同测试、真实工件生产 E2E 和实机质量/资源三类；可使用经生产入口摄入的无 PII 合成记忆，不能把 Stub、模拟器性能或历史模型审批外推为新 LLM 的证据。任务与 DEF-78-001 本次仍未完成，后续验证任务只消费证据。
...
### 4.0l 照片按需理解与无需手写描述的创作闭环（ADR-025）

**当前执行优先级（2026-09-09 人类明确）**：先完成真实功能闭环，再优化精确度/文采。已获研究授权的 SmolVLM-256M 继续原生转换和接线可行性验证；其首轮质量失败保留为待办，不作为转向更大模型的理由。Qwen3.5 下载提议暂缓。当前研究授权不等于生产工件批准，最终质量/资源/发布门禁保持未通过。

状态初始 backlog，依赖 4.0k；不是已暂缓的诗歌文采调优。先形成端侧图像理解候选和可审阅工件/预算，取得确切审批后实现。交付用户进入照片详情或选择照片创作后按需有界处理、OCR 正文和机器画面描述的独立持久化、新旧照片幂等按需处理、用户可选校正、真实状态/恢复、创作与报告素材接线，以及授权/来源版本/D-005 全链路。完整合同及验收矩阵见 ADR-025、US-ING-004 AC-6~9 和 US-SYN-003 AC-7。

4.0k/PR #79 保持文本运行时功能 review，其已有质量/实机/覆盖率门禁仍归原任务。4.0l 不得以给图片手写描述或仅 OCR 的测试代替无字照片生产流程。4.2~4.9、4.10、4.14 增加该依赖；测试/发布任务只消费实现，不补写视觉能力。主要测试规划路径：`EchoTests/Phase4/4.0l_PhotoUnderstandingCreationTests.swift`（尚未创建）。模型下载/替换和实施尚未启动。

## 4.0m 统一创作库

用户于 2026-09-09 批准在当前 4.0l 已提交分支基础上开发。范围及 AC 见 ADR-026、US-SYN-003 AC-8~10。4.0l 保留 review 与未通过项；不等待模型质量优化，不降低最终门禁。

...
| 4.0l | 照片自动理解与无需手写描述的创作闭环 | 本地画面描述/OCR 正文、独立派生存储、新旧照片处理、手动创作/报告输入与 D-005 | 4.0k |
...
### 4.0l 照片自动理解与无需手写描述的创作闭环（ADR-025）

状态初始 backlog，依赖 4.0k；不是已暂缓的诗歌文采调优。先形成端侧图像理解候选和可审阅工件/预算，取得确切审批后实现。交付照片摄入后自动有界处理、OCR 正文和机器画面描述的独立持久化、新旧照片幂等补处理、用户可选校正、真实状态/恢复、创作与报告素材接线，以及授权/来源版本/D-005 全链路。完整合同及验收矩阵见 ADR-025、US-ING-004 AC-6~9 和 US-SYN-003 AC-7。

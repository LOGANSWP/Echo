...
## CPU compatibility diagnostic conversion

The same pinned SmolVLM source is converted with FLOAT32 decoder compute and weights; FP16 KV states and the approved vision model are retained. No additional upstream model or dependency is introduced. This supplemental artifact requires explicit local App approval.

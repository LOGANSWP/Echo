...
**4.0k 生成验收（ADR-023）**：双模拟器继续用于 UI/兼容性检查，不能替代真实工件在获批实机矩阵上的内存、包体、热状态、生成/取消时延和质量证据。生产 no-fixture E2E 允许通过真实摄入入口写入无 PII 合成测试记忆，但模型/provider/解析/LanguageAligner/持久化必须是生产路径。合同测试中的模型替身需单独标注。生成验收覆盖完整 token 预算、逐层 allow-list、实际 coverage、派生删除关系、JSON 先解析/正文语言检测、每调用≤1 语言重试及总调用预算、typed fallback 不 publication、同 taskId 恢复重放与身份变化拒绝。真实模型质量按预先冻结的双语分层样本及标准评估，不能要求跨设备正文逐字一致，也不能把 N/A 重试率或降级提示计作成功。现有 no-media、签名、覆盖率与隐私门禁不变。
...
- 4.0h 验证类型化来源矩阵：PhotoKit photo/video 分开断言内容可用性与删除能力；Share Extension text 展示 canonical，audio 只展示转写且所有 Share 来源不可删、不猜测 host App。删除能力要求 UserPolicy、authorized/limited-visible、资产可获取与 `PHAsset.canPerform(.delete)`；iCloud-only 未下载不与删除能力混为一谈。请求前持久化同一 journal 的 `intentKind=photoLibraryAndEcho + sourceDeletionState=prepared + phase=.planned`，只有 `confirmedDeleted` 解锁 D-005。覆盖取消/拒绝/失败、full-present/full-missing、limited-visible/limited-hidden、撤权、每个崩溃点、同 memory 并发 intent、真实结构化审计，以及 ExcludedAssets 提示实际展示前后 `userNotified=false→true`
- 4.0i 验证版本化受限 JSON envelope 的正常/未知版本/畸形/超限路径，opaque source ID 精确 allow-list，无 round-robin fallback，以及 `[SourceAnchor]` + `cited/noSource/partialNoSource` 多引用语义；断言 UI 不把 provenance 表述为事实验证。导航在当前 UserPolicy 下进入稳定 Detail 或诚实不可用。复制、plain text、Markdown、PDF 分别覆盖当前授权复验、撤权阻断、离线来源保留诚实标记、引用保留和多页无截断。分享测试区分 payload 就绪但未呈现、准备失败、呈现失败、真实呈现、呈现后关闭和呈现后审计写入失败；验证 typed audit columns、幂等 L2 重试，以及零 activityType、目标 App、原文、assetID/sourceLocator
- 4.0j 按 ADR-021/022 验证：新安装默认开启的月/年持久开关、首条可用 canonical memory/升级迁移/关闭后重开的月/年独立 `eligibleFrom`，以及重开一种时另一种基线不变；首周期只读取 `max(start, eligibleFrom)` 之后的数据并标注 `partialBaseline`；刚结束完整周期与首次物化的时区/UTC 边界冻结；重复启动/前台/BG 并发时单周期 CAS；同结束边界先月后年且一次扫描只 claim 一个；持久 report/source/period 与 completion+audit 无挂起点事务故障矩阵；`noData`、L2 手动重试、system expiration/low-power/thermal 延后、D-005 invalidation 与 BGProcessingTask 接线。生产 provider 缺失时必须 `generationUnavailable`、零 claim/零 PendingOps；Stub 只验证 seam。4.0k 再以获批 bundled artifact 验证生产 Language Aligner、真实 `sourceBatches` 分批/分层、no-fixture 自动生成、重启和资源矩阵。fixture 报告不得计为生产生成证据，也不得断言精确后台时刻
...
## 4.0l 照片按需理解验收（ADR-025，2026-09-09 修订）

双设备真实 PhotoKit 摄入无 PII 无字照片 → 确认导入未创建理解任务 → 用户打开该照片详情或选择其创作 → 不编辑描述 → 本地按需理解 → 选模板生成 → 原照片来源跳转 → 正文复制/PDF/系统分享。含文字截图另验证 OCR，不能用其替代无字场景；描述必须来自当前真实图片和获批本地模型。覆盖旧照片按需处理、可选用户校正、中英、真实进度/失败、重启/取消、来源变更/撤权/删除及报告未准备 coverage。Fixture/手写照片描述只算合同或文本路径证据。继续使用无截图/视频的 Live Simulator Review；模型工件与实机资源单独验收。

按需触发专项验证：停留首页、导入、同步及前后台切换不增加理解任务；打开一张未准备照片仅创建该照片任务，重复访问有效素材不推理，失败不自动重试。旧自动 queued 意图及对应 TaskProgress/PendingOperations 迁移清理，已完成素材和新 onDemand 任务保留。实际证据见 [4.0l 按需验证](../05-planning/4.0l-on-demand-evidence.json)。
...
验证提交后离开创作页、在 Echo 内浏览、完成后库中读取及重启持久；失败、取消、中断、重复、撤权、来源删除和磁盘失败定向测试。真实模型失败必须显示失败，不以 fixture 替代生产验收。

**4.0m Live Review 修正（2026-09-09）**：创作库行导航只读状态，不触发重试；行内重试/取消按钮独立响应。从创作库进入详情后，查看创作库返回原列表，不再推入新的创作库。验收覆盖点击失败条目仍保持失败、显式重试才入队、往返导航无卡死。

### 4.0m 新创作交互回归
必须从真实照片详情选择模板并开始生成，验证打开/关闭创作库 sheet、切换设置标签后返回、返回照片详情/首页，再进入创作库仍显示同一任务。仅从创作库重试不足以覆盖新创作导航路径。

...
- 4.0j 按 ADR-021 验证：新安装默认开启的月/年持久开关、首条可用 canonical memory/升级迁移/关闭后重开的月/年独立 `eligibleFrom`，以及重开一种时另一种基线不变；首周期只读取 `max(start, eligibleFrom)` 之后的数据并标注 `partialBaseline`；刚结束完整周期与首次物化的时区/UTC 边界冻结；重复启动/前台/BG 并发时单周期 CAS；同结束边界先月后年且一次扫描只 claim 一个；持久 report/source/period 与 completion+audit 无挂起点事务故障矩阵；版本化有界聚合、`noData`、可选分区省略；L2 扫描跳过与用户手动重试；system expiration/low-power/thermal 延后；用户取消的精确 taskId 恢复；用户删报告、Memory 删除/全量撤权 D-005 invalidation 与 v1 禁止重建；BGProcessingTask 注册、permitted identifier、expiration/completion/resubmission。fixture 报告不得计为调度证据，也不得断言精确后台时刻

...
- ADR-017 将后续生产接线拆分：`4.0e` 负责 `MemoryUserEdit`、安全重索引和持久 conflict；`4.0h` 负责真实来源解析与 PhotoKit 删除 saga（恢复细则由 ADR-019 收紧）；`4.0i` 负责 allow-list 校验的 source anchor、稳定 Detail 路由与 `.creationSharePresented`；`4.0j` 负责持久 earliest-eligible 叙事报告调度。ViewModel 只转发 intent 和映射状态，不直接持有数据库、PHAsset 或删除 journal。
- Detail 经 `FocusSourceResolver` 在当前 UserPolicy 下分别映射 `contentAvailability`、presentation 与 `sourceDeletionCapability`；解析失败不回退 bundled sample。只有当前范围内可获取且 `PHAsset.canPerform(.delete)` 的 PhotoKit photo/video 暴露原始删除动作；内容未下载不自动等于不可删除。Share Extension 文本读取 canonical 内容，音频因原件不持久化只展示转写，所有 Share Extension 来源只允许从 Echo 移除且不得猜测 host App。limited-hidden/撤权不可见不等于删除。Creation 只消费经输入 allow-list 校验的 grounded source anchors；Notes 交接使用系统 share sheet 且不产生 Echo 可验证的“已保存”状态。Translation 继续作为 Detail 内 cache-first 的展示层能力，NLTagger `<0.9` 或语言对不支持时保留原文。

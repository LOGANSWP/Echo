...
- ADR-017/022 将后续生产接线拆分：`4.0e` 负责 `MemoryUserEdit`、安全重索引和持久 conflict；`4.0h` 负责真实来源解析与 PhotoKit 删除 saga（恢复细则由 ADR-019 收紧）；`4.0i` 负责 ADR-020 定义的受限结构化 source anchors、稳定 Detail 路由、当前授权下的复制/导出/分享与 `.creationSharePresented`；`4.0j` 按 ADR-021 负责默认开启的月/年持久开关及相互独立的 `monthlyEligibleFrom` / `yearlyEligibleFrom` 基线、完整周期/冻结时区、单周期 CAS、持久 report/source、原子 completion+audit publication、noData/L2/资源延后、D-005 invalidation 与 generator=nil 时 claim 前 fail-closed；`4.0k` 才负责获批 bundled LLM、生产 provider、Language Aligner 与真实有界分批/分层生成。4.0k 进一步遵守 ADR-023 的 token/执行预算、逐层来源与派生依赖、JSON/正文语言分离、typed fallback 不 publication、无草稿恢复重放和实机证据边界。Creation ViewModel 只投影持久报告和真实调度/能力状态并转发 intent，不直接持有数据库、BGTask、周期 claim、PHAsset、删除 journal 或第二份授权真相。
- Detail 经 `FocusSourceResolver` 在当前 UserPolicy 下分别映射 `contentAvailability`、presentation 与 `sourceDeletionCapability`；解析失败不回退 bundled sample。只有当前范围内可获取且 `PHAsset.canPerform(.delete)` 的 PhotoKit photo/video 暴露原始删除动作；内容未下载不自动等于不可删除。Share Extension 文本读取 canonical 内容，音频因原件不持久化只展示转写，所有 Share Extension 来源只允许从 Echo 移除且不得猜测 host App。limited-hidden/撤权不可见不等于删除。Creation 消费 `[SourceAnchor]` 与 `cited/noSource/partialNoSource`，allow-list 只证明 provenance；复制/导出/分享由 export coordinator 在准备前重新执行当前 UserPolicy + PrivacyCheckpoint，撤权阻止派生正文交接；按 ADR-024，所有外部格式默认只含标题和正文，内部引用与无来源状态仍保留。只有系统分享控制器真实呈现回调才是 `sharePresented=true` 证据；Notes 交接不产生 Echo 可验证的“已保存”状态。Translation 继续作为 Detail 内 cache-first 的展示层能力，NLTagger `<0.9` 或语言对不支持时保留原文。
...
### 照片按需理解状态投影（4.0l / ADR-025）

仅用户打开该照片详情或选择该照片进行 AI 创作时，才按需准备画面描述/OCR 创作素材；导入、同步、前后台及自动报告扫描不发起全相册理解。有效结果复用，失败仅显式重试。 Detail 显示独立的 AI 画面描述、OCR 和可选用户校正；Creation 读取真实准备进度/可用/失败状态，正常路径无需先填写描述，准备中可等待/继续。ViewModel 不自行生成描述、拼接占位事实或保存第二份素材真相。4.0k 当前无文本提示是能力缺失时的临时诚实反馈，不是最终产品流程；Core 持久化/队列/模型能力由 4.0l 单独交付，Focus/Task no-masonry 和系统分享边界保持不变。
...
首页入口通往 Apple 原生 List 任务列表，完成作品使用 Focus 单列阅读与现有来源/分享动作。列表与详情共享 echo-memory-canvas，禁止 masonry。页面只订阅应用级任务，离开页面不取消领域执行。
...
### 创作资源等待（PR #81）
创作库领域 deferred 映射为 CreationViewModel.waitingForResources 和列表“等待设备资源”，显示明确的“继续”动作，不映射成 L2 错误页或持续运行转圈。进入记录只读；继续由领域 Actor 用当前输入、授权和语言重新入队。

...
### 手动中文诗歌提示词预算修复
2026-09-09 16 Pro 实测旧提示词单次总耗时 120.50 秒，其中 prefill 115.18 秒、仅输出 1 token，真实 deadline。v13 仅为 manualCreation 的中文诗歌使用紧凑四行指令、五至八字建议和结构示例，移除长独立示范；保留来源不可信边界、request alias 映射与四段 grammar。标准报告/归约与非手动诗歌提示不变，模型工件与预算不变。Prompt 版本变化纳入既有恢复身份。功能与文采质量分别验收。

v13 四行紧凑版本在 iOS26 实测 70 秒、94 output tokens，但 iOS18 再次达到 120.47 秒（prefill 27.74 秒、decode 76.33 秒、73 tokens）。v14 将手动中文诗歌目标收敛为规格 US-SYN-003 AC-1 已允许的三行，提示每行五字，grammar 按该请求范围严格匹配三段并保留来源引用。标准诗歌仍四行；请求范围和 Prompt 版本进入既有身份。此变更减少输出工作量，不增加预算，模型正文不经客户端拆分。

v14 三段方案的真实模型测试通过，但 16 Pro 狗图仍在 123.42 秒超时（44 tokens）。最终 v15 采用 US-SYN-003 AC-1 已允许的单段真实换行形式：模型直接在一个 text 中生成三行、只返回一次来源数组；使用既有有界段落 grammar，Pipeline 继续按真实换行校验 3–6 行，不由客户端插入换行。手动中文 Prompt 同步缩短，不包含示范；默认四段/报告协议不变。v13/v14 不作为最终修复通过证据。

最终实现采用 v17：放弃未通过的单段方案（v15 缺少换行，v16 输出超限），仅手动中文诗歌使用三段、每段最多五个 Unicode scalar 的解码约束；模型在约束下直接选 token，不在输出后截断或重排。默认四段与 prose grammar 不变。新增真实狗图描述、长度边界和未知来源拒绝测试。v17 合成模型回归 48.85 秒通过，诗意仍弱；实机/发布质量未通过。


### 2026-09-09：v18 手动中文短诗的紧凑 JSON 排版
红圆合成照片已具备真实机器描述，但 v17 在 16 Pro 手动重试用满 256 output tokens；独立 17 Pro 相同输入也复现 outputLimit。只记录结构计数的诊断显示 548 输出字节中有 473 字节空白，未完成 JSON；这不是输入过长或照片理解未就绪。

v18 仅在原有 compact manual zh-Hans poem scope 禁止 JSON 字符串外的格式空白，Prompt 明确要求紧凑 JSON。正文中的空格仍由原有 text parser 接受；不截断/改写模型正文，不制造诗行或来源，保留三段与每段五 scalar 限制，普通报告/标准诗歌语法不变。日志只记录字节数、空白字节数及布尔完成/scope，不记录正文。模型、256-token 输出限额、120 秒模拟器单调用预算均未增加。

创作库同时单独持久化 output-limit，列表和详情用双语文案说明模型未完成输出，PendingOperations 仍仅手动重试。旧失败记录不自动重放或改为成功。

新增红圆回归针对稀疏画面描述的功能完成、格式、语言和引用；重复诗行记录为后续质量待办，不作为该新增功能回归的断言。既有狗图及其他主题的 distinct-line 检查保持启用，既有 CI/发布/实机门禁不变。真实 UI publication 与重开结果另记于 4.0m 实现证据。

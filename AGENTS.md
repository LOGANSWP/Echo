...
**版本**：v5.3  
**生效日期**：2026-06-29  
**适用对象**：所有参与 Echo 项目开发的 AI Agent（OpenCode 桌面版 / Codex / Cursor / Claude）及人类开发者  **优先级**：本规约优先于任何 Agent 的默认行为。当本规约与 Agent 默认行为冲突时，以本规约为准。  
...
| **OpenCode 本地配置**  | `.opencode/config.toml`                                | OpenCode 行为配置（可选，桌面版可通过 UI 配置） |
...
| **配置 OpenCode**       | `.opencode/config.toml`                                | PR 模板、审查人、合并策略                          |

### 0.3 Agent 文档读取规范

1. **启动时**：Agent 自动加载本文件（`AGENTS.md`），并**必须**读取 `docs/05-planning/task-status.json` 以确认当前任务状态。
...
│   └── GoldenDataset/
├── .opencode/
│   └── config.toml              # OpenCode 配置文件（可选，桌面版可通过 UI 配置）
├── docs/                        # 📚 项目文档中心（详见第0章）
│   ├── INDEX.md                 # 文档摘要索引
│   ├── 01-spec/
│   ├── 02-architecture/
...
1. **读取 `task-status.json`**：确认当前阶段（`current_phase`）和状态
2. **确认当前任务**：找到该阶段中第一个 `status: ready` 的任务
3. **检查依赖**：确认该任务的所有 `dependencies` 均已标记为 `done`
4. **输出任务信息**：
   ```
   当前阶段：Phase X - [阶段名称]
   当前任务：[任务ID] - [任务标题]
...
### 15.2 OpenCode 配置文件（桌面版）

OpenCode 桌面版主要通过 UI 界面进行 GitHub 配置。配置文件 `.opencode/config.toml` 是**可选的**，用于补充或覆盖 UI 配置。
...
> **说明**：PR 描述内容依据 `AGENTS.md` §3.3 中的模板定义，无需单独配置文件。桌面版用户应在 App 设置中完成 GitHub OAuth 登录，配置文件仅作为补充。如桌面版 UI 已覆盖相关设置，以 UI 为准。
...
| v5.2 | 2026-06-29 | 适配 OpenCode 桌面版：移除 `.github/workflows/opencode.yml` 相关内容；更新适用对象、文档目录、目录结构、配置说明；删除 §15.6（GitHub Actions 工作流） | AI 架构师 |
| v5.3 | 2026-06-29 | 移除 `.opencode/pr-template.md` 相关引用：删除 §0.1 文档目录中的 PR 描述模板条目；删除 §10.1 目录结构中的 pr-template.md；删除 §15.2 配置中的 `description_file` 行；移除 §15.7（原 PR 描述模板章节，已由 §3.3 覆盖）；更新 §15.2 添加说明：PR 描述内容依据 §3.3 | AI 架构师 |

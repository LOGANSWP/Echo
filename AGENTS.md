...
**文档来源**：所有项目文档已迁移至本地 `docs/` 目录，Codex 应直接读取本地文件，不再依赖 Notion。

---

## 0. 项目文档与快速索引

所有项目规格、架构、实现规范、AI Native 开发指南及计划均存放于项目根目录下的 `docs/` 文件夹中。Codex 在执行任何编码任务前，**必须**先查阅相关文档并引用原文。
...
| **查阅开发计划**        | `docs/05-planning/开发计划安排文档.md`                 | 里程碑、时间线                                     |

### 0.3 Agent 文档读取规范

1. **启动时**：Codex 自动加载本文件（`AGENTS.md`），并建议**同时读取 `docs/INDEX.md`**（如存在）以建立全局认知。
...
| 本地文档（`docs/`） | 规格读取       | Codex 通过 `read_file` 工具直接读取 |
...
```mermaid
flowchart TD
    A[收到任务] --> B[Step 1: 查阅 §0.2 任务类型映射表]
    B --> C[确定应读取的文档和章节]
    C --> D[Step 2: 使用 read_file 读取相关章节]
    D --> E[逐字粘贴 AC/规则原文]
    E --> F[Step 3: 基于引用的 AC 编写测试用例]
    F --> G[Step 4: TDD 增量实现]
    G --> H[Step 5: 生成 AC 覆盖表]
    H --> I[Step 6: 提交 PR，附自检清单]
```
...
1. **查阅映射表**：根据任务类型查阅 §0.2 的任务类型 → 文档快速索引，确定应读取的文档和章节。
2. **读取原文**：使用 `read_file` 工具读取对应文档章节（而非整篇文档）。
3. **原文引用**：在回复中，**逐字粘贴**本次涉及的验收标准（AC）原文或架构约束原文。
4. **禁止推断**：严禁使用“根据常规做法，我认为应该...”之类的推断。如果 AC 描述模糊，Agent 必须在此步骤停止编码，并向人类提出澄清问题。
...
| v4.9 | 2026-06-27 | 新增 §0.2 任务类型 → 文档快速索引；新增 §0.3 Agent 文档读取规范；优化防幻觉工作流；更新自检清单增加文档引用检查 | AI 架构师 |
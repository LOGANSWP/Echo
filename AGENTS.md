...
**4.0l 模拟器手动创作预算例外（2026-09-09 人类批准）**：仅 `DEBUG && targetEnvironment(simulator)` 的手动创作使用单调用 120 秒、同请求总计 240 秒（含既有最多一次语言重试）。实机、Release、自动月报/年报维持原预算；模型工件、token/内存上限、来源/语言校验和发布门禁不变。详见 `docs/05-planning/4.0l-simulator-time-budget-approval.json` 与 ADR-023。
...
**功能闭环优先（2026-09-09 人类明确指示）**：当前项目初期构建以真实功能链路为第一目标；描述精确度、事实细节、文采与模型质量排名进入后续优化，不作为继续研究转换、接线和功能验证的前置条件。特别是 4.0l，已获研究授权的候选只要能真实消费图片并产生可追踪输出，就可继续原生功能可行性验证，不能仅凭小样本语义或双语质量失败就自动淘汰并转向更大模型下载。必须分别报告“功能验证结果”和“质量待办”，保留真实失败，不能把功能通过写成完整质量/实机/发布通过。零网络、授权/删除、来源身份、真实模型、有限预算、失败不伪装成功等边界继续执行；最终用户可见输出仍遵循 preferredLanguage。此优先级不修改 CI/发布门禁或授予新模型/生产工件批准，现有合法研究授权继续有效。

**4.0l 按需触发（2026-09-09 人类明确指示）**：导入仅建立既有检索索引；仅用户打开照片详情或选择照片进行 AI 创作时才为该照片准备画面描述/OCR 创作素材。导入、同步、前后台及自动报告扫描均不得触发全相册理解。已有有效素材复用，失效素材待下一次访问重新处理，L2 仍仅手动重试。自动报告只消费已准备素材，原 coverage/noData/周期边界保留。迁移清除旧自动批量未完成队列，保留完成素材和新按需任务的恢复语义。
...
| v5.58 | 2026-09-09 | 用户明确初期功能闭环优先：模型精确度与文采不作为研究转换/接线前置门槛；功能证据与质量待办分别记录，保留隐私、来源、语言、工件审批与最终发布门禁。 | Codex |

| v5.59 | 2026-09-09 | 用户修订 4.0l 为详情/创作按需图片理解，取消导入与前台全量理解；旧自动队列迁移清理、有效素材复用和报告消费边界同步。 | Codex |

| v5.60 | 2026-09-09 | 用户批准仅 Debug 模拟器手动创作时间预算增补（120 秒/调用、240 秒/请求）；自动报告、实机与发布标准不变。 | Codex |

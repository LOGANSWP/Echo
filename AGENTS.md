...
**Codex 行为要求**：
- 在编写任何业务代码之前，必须根据任务类型读取相关文档，并在回复中**逐字粘贴**涉及的验收标准（AC）原文。
- 禁止凭“记忆”或“训练数据”猜测规格内容。
- 如文档描述模糊，必须停止编码并向人类提出澄清问题。
...
1. **锁定上下文**：根据任务类型，使用 `read_file` 工具读取 `docs/` 目录下对应的规格文档。例如：
   - 涉及用户故事 → 读取 `docs/01-spec/用户故事与验收标准规格书.md`，定位到具体故事编号（如 `US-PRV-001`）。
   - 涉及架构设计 → 读取 `docs/02-architecture/架构设计文档.md` 对应章节。
   - 涉及避坑规则 → 读取 `docs/03-implementation/开发避坑与关键注意点手册.md` 相关条目。

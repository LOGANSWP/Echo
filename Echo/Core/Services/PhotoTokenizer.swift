...
nonisolated enum PhotoTokenError: Error {
    case configuration, vocabulary, inputBudget, encoding
}

nonisolated struct PhotoTokenPair: Hashable, Sendable {
    let left: Int
    let right: Int
}

nonisolated struct PhotoTokenMerge: Sendable {
    let rank: Int
    let token: Int
}

nonisolated struct PhotoTokenizer: Sendable {
    ...
    init(url: URL) throws {
        ...
        let required = [
            "<|im_start|>": 1, "<end_of_utterance>": 49279,
    ...
    func encodePrompt(_ text: String) throws -> [Int] {
        ...
        else { throw PhotoTokenError.inputBudget }
